...
class GPActive():
    ...
    def gp_train(self):
        ...
        try:
            ...
            t_other = [gpflow.kernels.Matern52(input_dim=1, active_dims=[2])*gpflow.kernels.Periodic(input_dim=1, active_dims=[2]) for i in range(5)]
            time = t_matern
            for i in t_other:
                time = time + i
        ...
        for curr_day in range( self.total_days - self.context_days ):
            ...
            if flag:
                ...
                if self.to_test:
                    ...
                    self.rmse_error[curr_day] = -1 
                    self.mae_error[curr_day] = -1
            else:
                self.rmse_error[curr_day] = -1
                self.mae_error[curr_day] = -1
