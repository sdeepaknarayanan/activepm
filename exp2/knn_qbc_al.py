...
class ActiveLearning():
    ...
    def querybycommittee(self):
        ...
        for itr in range(1, self.test_days + 1):
            ...
            if itr % 10 == 0:
                ...
                if not self.gp_choice:
                    ...
                    temp_store_path = f"results/{self.train_days}/intermediate_knn/{self.fname[0]}_{self.fname[1]}/{self.current_day}"
        ...
        if not self.gp_choice:
            store_path = f"results/{self.train_days}/final_knn/{self.fname[0]}_{self.fname[1]}"
    ...
    def random_sampling(self):
        ...
        for seed in range(self.number_of_seeds):
            ...
            for itr in range(1, self.test_days + 1):
                ...
                temp_store_path = f"results/{self.train_days}/intermediate_random_knn/{self.fname[0]}_{self.fname[1]}/{seed}/{self.current_day}"
        ...
        store_path = f"results/{self.train_days}/final_random_knn/{self.fname[0]}_{self.fname[1]}"
        if not os.path.exists(store_path):
            os.makedirs(store_path)
